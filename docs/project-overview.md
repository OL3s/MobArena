...
- Weather state lives in `scripts/resources/WeatherState.cs` as a Godot `Resource`; the shared runtime instance is stored on `SaveNode.WeatherState` so town and arena render the same current weather.
