...
- Frontend environment visuals live in `scenes/components/environment/EnvironmentVisualOverlay.tscn`. It has separate enums for time of day and weather so Night is a phase-driven lighting layer, while weather such as Clear/Sun/Rain can be composed independently later. Town currently wires the overlay to `TownPhaseState` for the Night tint and leaves weather as Clear.
