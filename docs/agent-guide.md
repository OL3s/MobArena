...
- Roster management should stay centered on an in-town open area where current gladiators are visible idling and can be dragged into management sections. Aim for Training Hall on the left, healing/rest on the right, and clear access to market functions for new gladiators and equipment work.
...
- Healer and Training Hall effects are explicit phase-transition work, not continuous time. They execute once on Day -> Night and once on Night -> Day through `CompanyRunData.ExecutePhaseBuildingWork`.
...
- Company run state belongs in `scripts/resources/CompanyRunData.cs` and is separate from career totals. Track frequent mutable values there, including current gold, the current active `GladiatorData` list, current cemetery list, equipment inventory, town assignments, arena control assignments, and current run mob kills. New company runs should go through `SaveNode.StartNewCompanyRun()`, which adds two default gladiators and updates `CompanyCareerData` through `CompanyRunData.AddGladiator`. `AliveGladiators` is derived from the active gladiator array. Use its methods for changes such as `AddGladiator`, `AddGold`, `TrySpendGold`, `AddMobKilled`, and `KillGladiator`.
...
- `GladiatorData` owns gladiator condition values and caps. `Exhaustion` is the only exported 0-10 management condition. New default gladiators roll exhaustion in the 6-10 range. Low exhaustion caps recoverable health and stamina; values at 5 or above apply no cap penalty, then the multiplier scales from 1 down to 0 between 5 and 0. Current health and stamina should be restored through gladiator methods so they respect those recoverable caps.
...
- Use `PhaseTransitionController.CompleteArenaDay` for Day -> Night after arena and `PhaseTransitionController.AdvanceToNextDay` for Night -> Day from the HUD. Use `TownPhaseState` APIs for direct phase UI behavior: `GetDayLabel`, `GetPhaseLabel`, `GetPhaseProgressValue`, `GetPhaseProgressMax`, and `CanAdvanceToNextDay`.
...
- Town HUD top resources focus on company status, spendable gold/fame, and roster Risk warnings. Risk display should use `GladiatorData.GetRiskStatus(...)` so one risky gladiator produces one icon/count only: exhausted, low health, or critical for both. Low-health uses `SettingsConfig.LowHealthWarningRatio`.
...
- Building overlays based on `BuildingOverlayPanel.tscn` can show assigned gladiators with `ShowAssignedGladiators = true` and an `AssignmentLocation`. Arena, Healer, and Training Hall overlays use this to show `[grab icon][gladiator][gladiator n+1]` inside the overlay, not on the town building sprite. Dragging one of those portrait buttons closes the overlay and calls `RosterYard.StartGladiatorDrag`; if the drag is released over no valid target, `RosterYard` returns the gladiator to the courtyard through `CompanyRunData.TryMoveGladiatorToCourtyard`.
...
- Item and gladiator purchases must keep the original `Cost`/`InitialCost` on the runtime resource. Resale pricing is computed at sale time: item sales use `CompanyRunData.GetSaleValue(ItemData)`, and gladiator sales use `GladiatorData.GetMarketSaleValue()`, both returning half of the original price.
