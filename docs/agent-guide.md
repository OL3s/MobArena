...
- Town UI should represent boss pressure as time remaining before a mandatory boss fight deadline, not as a simple number of arena fights completed.
- Town phase is intended to use real time with Paused, Slowed, Normal, and Fast speed states. The bottom UI should put left/right speed arrows and a speed toggle on the left, then show current day and a sun-to-moon day progress bar for readability. Boss deadline belongs with the bottom timeline.
...
- Use `TownTimeState` APIs for time: `TickOneSecond`, `AdvanceMinutes`, `IncreaseSpeed`, `DecreaseSpeed`, `TogglePaused`, `GetSpeedLabel`, `GetDayLabel`, `GetDigitalTimeLabel`, `GetDayProgressValue`, `GetDayProgressMax`, `GetDayPhaseLabel`, `IsTownOpen`, `IsTownSleeping`, `AreStoresOpen`, `GetBossProgressValue`, `GetBossProgressMax`, `GetBossDeadlineLabel`, and `IsBossDue`.
