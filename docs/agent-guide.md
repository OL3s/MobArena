...
- The current implementation focus is working rations and markets. Use existing run state rather than market-local state: spend gold with `CompanyRunData.TrySpendGold`, mutate `CompanyRunData.Rations`, and emit/run through existing `RunChanged` paths so town HUD totals and starvation counts refresh.
- Starvation warnings should remain one popup per day with the affected count stored in `TownTimeState`, not one popup per starving gladiator. Popups that should block time should use `GlobalOverlay` popup helpers with `pauseGameUntilClosed: true` so pause/resume behavior stays generic.
...
- Autosave should remain lightweight and deliberate. Current save triggers are company create/edit, app exit through `SaveNode._ExitTree`, pressing Back from town to main menu through `Town.cs`, and town day rollover at 00:00 from `TownHud` after `GameTimeController.TickOneSecond` advances time.
...
- Town HUD top resources are split into a Wealth panel and a Supply panel. Wealth shows current gold and current fame with separate icons; fame uses `assets/ui/icons/fame.svg`. Supply shows a 2x2 ration grid for poor, common, fine, and total ration counts.
...
- The Market town building uses `SellDroppedPayloads = true`. Dropping an item, gladiator, or ration onto the Market sells it through `CompanyRunData.TrySellItem`, `TrySellGladiator`, or `TrySellRation`, and the building shows a gold preview through `SetTownDragDropPreview` while dragging over it. Keep real sell mutations in run/resource APIs, not in UI-only scripts.
...
- Dropping rations onto roaming `RosterYardGladiator` nodes feeds that gladiator through `CompanyRunData.TryFeedGladiatorRation`. Drop receivers must validate that payloads still exist in current run state before mutating state; print clear `GD.PushError` messages when a dragged item, gladiator, or ration no longer exists in inventory/roster. This is intentional early bug detection for stale drag payloads and mismatched UI state.
...
- Item purchase already calls `ItemData.ApplyPurchasedValue`, halving item resale value when the market item enters company inventory. Future recruit purchases should call `CompanyRunData.TryBuyGladiator`, which calls `GladiatorData.ApplyPurchasedValue` before `AddGladiator`. Gladiator sale pricing must use `GladiatorData.GetMarketSaleValue()`, which returns half of `GetMarketValue()`; market value depends on initial cost, level-derived stats, vitals, provisions, and exhaustion. Rations are count-based and do not become individual purchased resources, so `RationInventory.GetMarketSaleValue` applies `/2` only when selling.
