...
Gladiators have two time-sensitive condition values from 0-10: provisions and exhaustion. Default gladiators start both at 8. Provisions represent food and thirst condition; better provisions should cost gold to maintain, 10 is best, and the value slowly decays over town time. If provisions reach 0, the gladiator dies. Exhaustion represents readiness after accumulated fatigue from repeated use; it should drop when a gladiator is used too often and recover during town time, encouraging roster rotation. Recovery limits use the lowest condition value, `min(exhaustion, provisions)`, so the two conditions do not stack. Values at 5 or above apply no cap penalty; below 5, the cap multiplier scales from 1 down to 0 as the lowest condition approaches 0. Health still cannot exceed the gladiator's base max health.
...
The town phase uses real time with x0, x1, x10, and x100 speed states. At x0, time is paused; x1 advances 1 in-game minute per real second; x10 advances 10 in-game minutes per real second; x100 advances 100 in-game minutes per real second. Time controls should sit on the left side of the bottom UI as left/right arrows that cycle through those speeds. For readability, the bottom timeline should focus on current day and digital time rather than showing week text. Champion deadline display belongs with the bottom timeline and should be time remaining before the required champion fight, not number of arena fights completed.

Time state should be represented by Godot `Resource`s so management systems can consume the same API without coupling directly to town UI. `TownTimeState` owns current day, digital time, current speed, champion deadline, time advancement, and speed changes. `GameTimeController` is a static service that owns the simulation tick flow and calls time progression systems after the clock advances. `CompanyTimeProgression` owns company and gladiator effects caused by passed time.
