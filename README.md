...
- `resources/gladiator_appearances/` and `resources/mob_appearances/`: Character appearance resources. Future equipment visuals should layer equipped gear from inventory onto gladiator body art so town and arena sprites visibly reflect what a gladiator has equipped.
...
Town buildings support `DisableWhenRosterEmpty`: when the active gladiator roster is empty, non-market buildings gray out, suppress hover popups, and ignore interaction so the Market remains the obvious Day recovery path for buying gladiators. Buildings can also opt into `DisableAtNight`; Arena and Market are disabled at Night so the player resolves after-fight city phase actions before starting or shopping again. `DisabledBuildingTexture` can provide a closed-building SVG for disabled state; Arena and Market use closed variants, while buildings without one fall back to gray modulation.
...
Future equipment visuals should use `GladiatorEquipmentData` to show equipped armor/weapons on gladiator body art in town and arena. Menu/card UI can keep using face portraits, but world/body presentation should visibly reflect equipped gear.
