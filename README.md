...
Settings state lives in `scripts/resources/SettingsConfig.cs`; the runtime instance lives on `SaveNode.SettingsConfig`. It currently stores the primary input preference for controls: auto-detect on/off and a default primary input of None, Keyboard, Touch, or Gamepad.
