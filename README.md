...
Town has a reusable `EnvironmentVisualOverlay` layered above the world and below HUD. It separates `TimeOfDayVisual` (`Day`, `Night`) from `WeatherVisual` (`Clear`, `Sun`, `Rain`) so future weather can combine with either day or night. Town currently drives the time tint from `TownPhaseState`; Night applies a dark blue overlay while weather defaults to Clear.
